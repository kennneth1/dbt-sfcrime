{{ config(
    ...
    unique_key='incidentNumber',
    pre_hook="
        delete from {{ this }}
        where weekEnding in (
            select distinct
                date_trunc('day', incidentDatetime + (6 - extract(dow from incidentDatetime)) * interval '1 day')
...
with staged as (
    ...
    from {{ ref('sf_crime_staging') }}
    {% if is_incremental() %}
        -- Only pull rows that are new or could affect existing weeks
